// components/DonorPartnerSection.tsx
...
const DonorPartnerLogo: React.FC<DonorPartner> = ({ name, logo, link }) => {
    ...
    return (
        <motion.div
            ...
            whileHover="hover"
            className="cursor-pointer" // Indicate it is interactive

        >
            <Link href={link || ''} target={link ? '_blank' : '_self'} rel={link ? 'noopener noreferrer' : ''}>  {/* Open external links in a new tab */}
            <Image
                src={logo}
                alt={`${name} Logo`}
...
//   return (
//     <section className="py-16 bg-gray-100" aria-labelledby="donor-partner-section-title">
